...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 29 20:02:10 2026

@author: aatkin
"""

"""
Peak Velocity Timing Analysis
==================anal============
Calculates when peak velocity occurs during phase 5 of each trial, expressed
as a percentage of the total phase-5 movement duration (normalized to 100).
Averages this value within participants for each combination of targetSize
and distance.

Method:
    1. Isolate phase-5 samples for each trial.
    2. Compute instantaneous velocity between each consecutive sample pair.
    3. Identify the sample pair at which peak velocity occurs.
    4. Express the timestamp of the peak velocity sample as a percentage of
       the total phase-5 duration:
           peak_velocity_timing (%) = (t_peak - t_start) / (t_end - t_start) * 100
       where t_start and t_end are the timestamps of the first and last
       phase-5 samples, and t_peak is the timestamp of the later sample in
       the pair at which peak velocity occurs.

Usage:
    python analyze_peak_velocity_timing.py --experiment_dir /path/to/experiment_folder
...
Optional arguments:
    ...
    --output_file      Name of the output CSV file (default: peak_velocity_timing_summary.csv)

Examples:
    # Single session
    python analyze_peak_velocity_timing.py --experiment_dir ./data
...
PEAK_VELOCITY_PHASE = 5         # Only use samples from this phase
...
def compute_peak_velocity_timing(trial_df: pd.DataFrame) -> float | None:
    """Compute when peak velocity occurs during phase 5, as a percentage of
    total phase-5 duration.

    Steps:
        1. Isolate phase-5 samples.
        2. Compute instantaneous velocity for each consecutive sample pair.
        3. Find the sample pair with the highest velocity.
        4. Express the timestamp of the later sample in that pair as a
           percentage of the total phase-5 duration.

    Returns None if there are fewer than 2 phase-5 samples, required columns
    are missing, all time deltas are zero, or phase-5 duration is zero.
    """
    required = [CURSOR_X_COL, CURSOR_Y_COL, TIME_COL, PHASE_COL]
    if not all(c in trial_df.columns for c in required):
        return None

    phase5 = trial_df[trial_df[PHASE_COL] == PEAK_VELOCITY_PHASE].copy()
    if len(phase5) < 2:
        return None
    ...
    # Compute velocities only for valid intervals; mark invalid ones as -inf
    # so they are never selected as the peak
    velocities = np.full(len(dt), -np.inf)
    velocities[valid] = np.sqrt(dx[valid] ** 2 + dy[valid] ** 2) / dt[valid]

    # Index of the peak velocity interval; t_peak is the later sample of that pair
    peak_idx = int(np.argmax(velocities))
    t_peak   = t[peak_idx + 1]

    timing_pct = (t_peak - t_start) / duration * 100
    return float(timing_pct)
...
def process_participant_dir(participant_dir: str, session_label: str,
    ...
    for fname in csv_files:
        ...
        # --- Compute peak velocity timing ---
        timing_pct = compute_peak_velocity_timing(trial_df)
        if timing_pct is None:
            print(f"  [WARNING] Could not compute peak velocity timing for {fname} "
                  f"— fewer than 2 phase-{PEAK_VELOCITY_PHASE} samples, zero duration, "
        ...
        trial_records.append({
            ...
            TARGET_SIZE_COL:           targetSize,
            DISTANCE_COL:              distance,
            "peak_velocity_timing_pct": timing_pct,
        })
...
def build_summary(all_trials: pd.DataFrame) -> pd.DataFrame:
    """Average peak velocity timing within participants for each
    ...
    mean_timing = (
        all_trials
        .groupby(["participant_id", TARGET_SIZE_COL, DISTANCE_COL])["peak_velocity_timing_pct"]
        .mean()
        .reset_index()
        .rename(columns={"peak_velocity_timing_pct": "mean_peak_velocity_timing_pct"})
    )

    mean_timing["condition_label"] = mean_timing.apply(
        lambda r: f"targetSize_{r[TARGET_SIZE_COL]}_distance_{r[DISTANCE_COL]}", axis=1
    )

    summary = mean_timing.pivot(
        index="participant_id",
        columns="condition_label",
        values="mean_peak_velocity_timing_pct"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Peak velocity timing analysis by target size and distance."
    )
    parser.add_argument(
        "--experiment_dir",
        required=True,
    ...
    parser.add_argument(
        "--output_file",
        default="peak_velocity_timing_summary.csv",
        help="Output CSV filename (saved in experiment_dir). Default: peak_velocity_timing_summary.csv"
    )
    ...
    # Report unique condition combinations found
