...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  2 09:25:31 2026
...
==========================
Calculates average velocity during phase 5 of each trial, then averages
within participants for each combination of targetSize and distance.

Average velocity is computed as:
    total Euclidean distance travelled across all phase-5 samples
    divided by total phase-5 duration (last timestamp - first timestamp).

Usage:
    python analyze_average_velocity.py --experiment_dir /path/to/experiment_folder
...
Optional arguments:
    ...
    --output_file      Name of the output CSV file (default: average_velocity_summary.csv)

Examples:
    # Single session
    python analyze_average_velocity.py --experiment_dir ./data
...
AVERAGE_VELOCITY_PHASE = 5      # Only use samples from this phase
...
def compute_average_velocity(trial_df: pd.DataFrame) -> float | None:
    """Compute average velocity during phase 5 of a trial.

    Average velocity = total path length across phase-5 samples
                       / total phase-5 duration.

    Returns None if there are fewer than 2 phase-5 samples, required columns
    are missing, or phase-5 duration is zero.
    """
    required = [CURSOR_X_COL, CURSOR_Y_COL, TIME_COL, PHASE_COL]
    if not all(c in trial_df.columns for c in required):
        return None

    phase5 = trial_df[trial_df[PHASE_COL] == AVERAGE_VELOCITY_PHASE].copy()
    if len(phase5) < 2:
        return None
    ...
    return float(total_distance / duration)
...
def process_participant_dir(participant_dir: str, session_label: str,
    ...
    for fname in csv_files:
        ...
        avg_vel = compute_average_velocity(trial_df)
        if avg_vel is None:
            print(f"  [WARNING] Could not compute average velocity for {fname} "
                  f"— fewer than 2 phase-{AVERAGE_VELOCITY_PHASE} samples, zero duration, "
...
def build_summary(all_trials: pd.DataFrame) -> pd.DataFrame:
    """Average velocity within participants for each targetSize x distance combination.
    ...
    mean_vel = (
        all_trials
        .groupby(["participant_id", TARGET_SIZE_COL, DISTANCE_COL])["average_velocity"]
        .mean()
        .reset_index()
        .rename(columns={"average_velocity": "mean_average_velocity"})
    )

    mean_vel["condition_label"] = mean_vel.apply(
        lambda r: f"targetSize_{r[TARGET_SIZE_COL]}_distance_{r[DISTANCE_COL]}", axis=1
    )

    summary = mean_vel.pivot(
        index="participant_id",
        columns="condition_label",
        values="mean_average_velocity"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Average velocity analysis by target size and distance."
    )
    parser.add_argument(
        "--experiment_dir",
        required=True,
    ...
    parser.add_argument(
        "--output_file",
        default="average_velocity_summary.csv",
        help="Output CSV filename (saved in experiment_dir). Default: average_velocity_summary.csv"
    )
