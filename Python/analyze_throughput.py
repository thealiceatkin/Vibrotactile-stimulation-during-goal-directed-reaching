...
================================
Calculates information throughput (TP) per trial, averaged within participants
...
Usage:
    python analyze_throughput.py --experiment_dir /path/to/experiment_folder
...
Optional arguments:
    ...
    --output_file      Name of the output CSV file (default: throughput_summary.csv)

Examples:
    # Single session
    python analyze_throughput.py --experiment_dir ./data
...
def compute_trial_metrics(trial_df: pd.DataFrame, fname: str) -> dict | None:
    ...
    then compute ID and TP.

    Returns a dict with keys: targetSize, distance, movement_time, ID, TP.
...
def process_participant_dir(participant_dir: str, session_label: str,
    ...
    Returns a DataFrame with one row per included trial containing:
        - participant_id
        - session
        - targetSize, distance, movement_time, ID, TP
    """
...
def build_summary(all_trials: pd.DataFrame) -> pd.DataFrame:
    """Average TP within participants for each unique ID value.
    ...
    mean_tp = (
        all_trials
        .groupby(["participant_id", "ID_rounded"])["TP"]
        .mean()
        .reset_index()
        .rename(columns={"TP": "mean_TP"})
    )

    mean_tp["ID_label"] = mean_tp["ID_rounded"].apply(lambda x: f"ID_{x:.4f}")

    summary = mean_tp.pivot(
...
def main():
    parser = argparse.ArgumentParser(description="Fitts' Law throughput analysis.")
    parser.add_argument(
        "--experiment_dir",
        required=True,
    ...
    parser.add_argument(
        "--output_file",
        default="throughput_summary.csv",
        help="Output CSV filename (saved in experiment_dir). Default: throughput_summary.csv"
    )
