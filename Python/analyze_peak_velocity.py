...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 29 11:56:03 2026
...
=======================
Calculates peak velocity on each trial from cursor position samples during
phase 5, then averages peak velocity within participants for each combination
of targetSize and distance.

Peak velocity is computed as:
    1. For each consecutive pair of phase-5 samples, compute Euclidean distance
       travelled and divide by the elapsed time between samples.
    2. The maximum of these instantaneous velocities is the trial's peak velocity.

Usage:
    python analyze_peak_velocity.py --experiment_dir /path/to/experiment_folder
...
Optional arguments:
    ...
    --output_file      Name of the output CSV file (default: peak_velocity_summary.csv)

Examples:
    # Single session
    python analyze_peak_velocity.py --experiment_dir ./data
...
def compute_peak_velocity(trial_df: pd.DataFrame) -> float | None:
    """Compute peak velocity from phase-5 samples in a trial.

    For each consecutive pair of phase-5 samples, computes instantaneous
    velocity as Euclidean distance / elapsed time. Returns the maximum.
    Returns None if there are fewer than 2 phase-5 samples, required columns
    are missing, or all time deltas are zero.
    """
...
def process_participant_dir(participant_dir: str, session_label: str,
    ...
    for fname in csv_files:
        ...
        if pd.isna(targetSize) or pd.isna(distance):
            print(f"  [WARNING] NaN in targetSze or distance in {fname} — skipping.")
            continue

        # --- Compute peak velocity ---
        peak_vel = compute_peak_velocity(trial_df)
        if peak_vel is None:
            print(f"  [WARNING] Could not compute peak velocity for {fname} "
                  f"— fewer than 2 phase-{PEAK_VELOCITY_PHASE} samples or missing columns. Skipping.")
            continue
...
def build_summary(all_trials: pd.DataFrame) -> pd.DataFrame:
    """Average peak velocity within participants for each targetSize x distance combination.
    ...
    mean_pv = (
        all_trials
        .groupby(["participant_id", TARGET_SIZE_COL, DISTANCE_COL])["peak_velocity"]
        .mean()
        .reset_index()
        .rename(columns={"peak_velocity": "mean_peak_velocity"})
    )

    mean_pv["condition_label"] = mean_pv.apply(
        lambda r: f"targetSize_{r[TARGET_SIZE_COL]}_distance_{r[DISTANCE_COL]}", axis=1
    )

    summary = mean_pv.pivot(
        index="participant_id",
        columns="condition_label",
        values="mean_peak_velocity"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Peak velocity analysis by target size and distance.")
    parser.add_argument(
        "--experiment_dir",
        required=True,
    ...
    parser.add_argument(
        "--output_file",
        default="peak_velocity_summary.csv",
        help="Output CSV filename (saved in experiment_dir). Default: peak_velocity_summary.csv"
    )
