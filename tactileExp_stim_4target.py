...
def setupTasks(cfg):
    ...
    vibrotactileStimTime = [1, 2, 3]
    cfg['state']['vibrotactileStimTime'] = vibrotactileStimTime

    strengths = [0, 30, 60, 90, 120]
    cfg['state']['strengths'] = strengths

    print("set condition values")

    # master list of all conditions, positon, size
    leftConditions = foldout(values = [[1, 0.5],  
                                    leftTargets, leftPrompt, vibrotactileStimTime, strengths],
                         names = ["targetSize", "targetPos", "promptPos", "vibrotactileStimTime", "strengths"])
    rightConditions = foldout(values = [[1, 0.5],
                                    rightTargets, rightPrompt, vibrotactileStimTime, strengths],
                         names = ["targetSize", "targetPos", "promptPos", "vibrotactileStimTime", "strengths"])
    ...
    # eight pseudo-randomized blocks
...
def runTasks(cfg):
    ...
    cfg['bin']['instruction'].text = 'On each trial, you will see a red dot. When a target appears, use the stylus to move the dot inside the target. When the target turns yellow, press the LEFT arrow key if you did not feel a vibration, or press the RIGHT arrow key if you did feel a vibration. Move or keep the cursor inside the new start position until the next target appears. When you are ready, press SPACE to start the task.'
    cfg['bin']['instruction'].draw()
    cfg['bin']['win'].flip()
    cfg['bin']['cursor'].fillColor = '#ff0000'
...
def runTrial(cfg):
    ...
    vibrotactileStimTime = cfg['state']['conditions']['vibrotactileStimTime'][condIdx]
    strength = cfg['state']['conditions']['strengths'][condIdx]
    print(targetSize)
    print(targetPos)
    print(promptPos)
    print(vibrotactileStimTime)
    print(strength)
    ...
    if trialNumber > 0:
        ...
        if trialNumber in (61, 121, 181, 241, 301, 361, 421):
    ...
    while runningTrial:
        ...
        if phase == 5:
            ...
            cfg['bin']['cursor'].draw()
            distance = ((cursorPos[0]-targetPos[0])**2+(cursorPos[1]-targetPos[1])**2)**0.5
            if deliverStim == False:
                if vibrotactileStimTime == 3:
                    if (time.time() > (leaveStartPositionTime + 0.5)):
                        # vibrotactileOnsetTime = time.time()
                        cfg['bin']['TVFU'].write(bytes('G2.', 'utf-8'))
                        deliverStim = True
                        print("delivered stimulation")
                        pass
            if distance < cfg['bin']['target'].radius:
    ...
    trialdata = { 'trial_idx'            : trial_idx,
                  'strength'             : strength,
                  'duration'             : duration,
                  'vibrotactileStimTime' : vibrotactileStimTime,
                  # 'vibrotactileOnsetTime': vibrotactileOnsetTime,
                  'startx_cm'            : startx,
                  'starty_cm'            : starty,
                  'targetx_cm'           : targetx,
                  'targety_cm'           : targety,
